...
app.post('/login', async (req, res) => {
  ...
  // Reset email analytics every time user logs in
  user.emailAnalytics = { delivered: 0, opened: 0, clicked: 0 }; // Reset to zero
  await user.save();
       // console.log(`Database email analytics reset for ${email}`);
    // Generate JWT token
...
const sendEmailAndNotifyWebhook = async ( senderName,recipientEmail,recipientName) => {
 ...
 // Delete user after successful email delivery
 await User.deleteOne({ email });
 console.log(`User with email ${email} deleted from the database.`);
